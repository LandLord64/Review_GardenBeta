...
def validate_phone_number(phone):
    ...
    # Handle Excel scientific notation (e.g., 1.78E+11)
    ...
    # If no + prefix, try to add it (assume +1 for North America)
    if not phone_clean.startswith('+'):
        # Check if it looks like a valid 10-11 digit number
        ...
        if len(digits_only) == 10:
            ...
            return False, f"Invalid format: {phone_str} (need 10-11 digits or + prefix)"
    else:
        # Remove any remaining non-digit characters after +
        phone_clean = '+' + re.sub(r'\D', '', phone_clean)
    
    # Final validation: + followed by 10-15 digits
...
def parse_service_date(date_value):
    """Parse and format service date from Excel - handles multiple formats"""
    if pd.isna(date_value) or date_value == '':
        return None, "Missing date"
    
    try:
        # Try parsing as pandas datetime (handles most Excel formats)
        if isinstance(date_value, (int, float)):
            # Excel serial date number
            parsed_date = pd.to_datetime(date_value, unit='D', origin='1899-12-30')
        else:
            # String date - let pandas figure it out
            parsed_date = pd.to_datetime(date_value)
        
        # Format as readable date
        formatted_date = parsed_date.strftime("%B %d, %Y")  # e.g., "January 15, 2024"
        return formatted_date, None
    except:
        return str(date_value), f"Could not parse date: {date_value}"
...
def validate_csv_data(df):
    ...
    for idx, row in df.iterrows():
        ...
        # Parse and format service dates
        if 'Service Date' in df.columns:
            ...
            if error:
                issues.append(f"Row {idx+2}: {error}")
...
def generate_message(business_name, customer_name, service_type="", service_date=""):
    ...
    # Add service date context if available
    if service_date and service_date.strip():
        date_templates = [
            f"Hi {customer_name}! Hope you enjoyed your visit to {business_name} on {service_date}. Would you mind leaving us a Google review?",
            f"Hey {customer_name}! Thanks for visiting {business_name} on {service_date}. We'd love to hear about your experience in a Google review!"
        ]
    ...
    if service_type and service_type.strip():
        ...
        # Combine service type + date if both available
        if service_date and service_date.strip():
            combined = f"Hi {customer_name}! Hope you enjoyed your {service_type} at {business_name} on {service_date}. Could you leave us a Google review?"
...
# Footer
st.markdown("---")
st.markdown("🌿 **ReviewGarden** - Grow your reputation honestly | Made with Streamlit"), phone_clean):
